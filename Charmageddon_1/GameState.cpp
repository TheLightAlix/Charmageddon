...
void GameState::AddPoints(int addedPoints) {
    if(!bIsInvincible){
        if (pointsThisLvl + addedPoints < 0){
            pointsThisLvl = -1;
            float copy = levelUpScaling - 0.5;
            int score = 0;
            for (copy; copy >= 1.0; copy -= 0.5){
                score  = score + basePointsToIncreaseLvl*copy;
            }
            points = score + pointsThisLvl;
        } else {
            points +=  addedPoints;
