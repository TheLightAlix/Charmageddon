...
} //prende le dimensioni della finestra e adatta l'output delle scritte all'altezza e alla larghezza della finestra.

ptr Menu::creazione_lista () {
//funzione che crea una lista nel programma, prendendo in input un file txt che si trova nella cartella del progetto.
    string name;
    int pos, score;
...
ptr Menu::newscore (ptr h, int nscore, string user) {
    //funzione che inserisce in classifica un nuovo score, col rispettivo user, posizione in classifica e score (livello), operando sulla lista;
    ptr p;
    p = h;
    while (p->next != NULL) {
        if (nscore > p->s) {
...
void Menu::presentazione() {
    //menu iniziale dove viene inserito l'user e vengono effettuate le scelte del colore e del tipo di macchina.
    window();
    gotoxy(x-24, y-2);
...
void Menu::post_menu (string user, int score) {
    //menu che viene mostrato quando si termina una partita. Se lo score è utile, viene inserito nella classifica chiamando la funzione new_score;
    system("cls");
    char key;
    window();
    gotoxy(x-9, y);
...
int Menu::crea_menu(int counter = 1) { //funzione principale che mostra tutte le scelte del menu e fa navigare l'utente all'interno di esso
    bool controllo = false;
    keybd_event(VK_SPACE, 0, 0, 0);
    keybd_event(VK_SPACE, 0, KEYEVENTF_KEYUP, 0);
    keybd_event(VK_SPACE, 0, 0, 0);
    ...
    while (controllo == false) {
        ...
        key = _getch(); //prende il carattere del tasto che l'utente da in input
        ...
        //viene stampata una freccia a seconda della posizione che il contatore ha nel menu, per permettere all'utente di orientarsi meglio.
